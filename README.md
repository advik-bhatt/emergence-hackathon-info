...
- **Emergence CRAFT MCP** (`nebius.emergence.ai/mcp`) — every SQL query in the pipeline was
  generated by CRAFT's `generate_sql`; none was hand-written. The responses (question,
  generated SQL, columns, rows) are recorded once to `fixtures/*.json` and replayed by
  `craft/cassette.py`, so the whole pipeline runs offline.
- **Nebius Token Factory** — the investigation's reasoning narrative runs on an
  OpenAI-compatible model (Kimi-K2) via Nebius. Set `NEBIUS_API_KEY` (see `.env.example`) and
  the agent drives the same tools live; without a key it replays an identical scripted trace,
  so the demo cannot fail open.
- **Verified, not hallucinated** — every number in the narrative is checked against the tool
  that produced it (`agent/narrative.py`), and a build fails loudly if the prose states a
  figure no tool computed.
